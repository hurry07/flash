...
function fixFrameRate() {
	if (doc.frameRate != 24) {
		var str11 = "The recommended frame rate for Flex components is 24fps. The current document frame rate is ";
		var str12 = "fps.\n\nWould you like to change the document frame rate to 24fps?";
		if (confirm(str11 + doc.frameRate + str12))
		{
...
function prepareDocument() {
	...
	if (doc.asVersion < minAsVersion || parseInt(doc.getPlayerVersion()) < minPlayerVersion) {
		var str4 = "Flex Components must target Flash Player ";
		var str5 = " and ActionScript ";
		var str6 = ".\n\nWould you like to change the document settings to target Flash Player ";
		var str7 = " and ActionScript ";
		var str8 = "?";
		if (!confirm(str4 + minPlayerVersion + str5 + minAsVersion + str6 + minPlayerVersion + str7 + minAsVersion + str8))
			return false;
	}
...
function applyLinkagePropertiesToSymbol(libItem, baseClass)
{
	...
	if (libItem != undefined)
	{
		...
		{
			var str21 = "Skipping Library item ";
			var str22 = ". It is not a MovieClip.";
			fl.trace(str21 + getShortName(libItem.name) + str22)
		}
		else
		{
			...
			var str22 = "Symbol ";
			var str23 = " can be used as a Flex component.";
			fl.trace(str22 + getShortName(libItem.name) + str23);
		}
	}
}

var additionalMessageArray = [
	"",
	"Add the 'FlexContentHolder' symbol to define the Flex content area."
];

function makeFlexComponent(baseClass, additionalMessageIndex)
{
	doc = fl.getDocumentDOM();
	...
	else
	{
		...
		if (additionalMessageIndex) {
			var additionalMessage = additionalMessageArray[additionalMessageIndex];
